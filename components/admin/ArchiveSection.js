...
const CATEGORY_LABELS = {
  ...
  vip_page:   { ar: "صفحات VIP", en: "VIP Pages" }, // 👈 جديد
};
...
export default function ArchiveSection({ lang = "ar" }) {
  ...
  const [files, setFiles]         = useState([]);   // للأرشيف أو صفحات VIP حسب الفلتر
  const [loading, setLoading]     = useState(true);
  const [msg, setMsg]             = useState("");

  // رفع ملف (للتصنيفات غير VIP)
  ...
  useEffect(() => {
    ...
    async function fetchList() {
      if (isVip) {
        ...
        const docs = snap.docs.map(d => ({ id: d.id, ...d.data() })); // يحتوي slug, clientName...
        setFiles(docs);
      } else {
        // عرض ملفات الأرشيف
        const qy = query(
  ...
  /* =========================
     Submit: حالتان
     - لو VIP: إنشاء صفحة فقط
     - غير VIP: رفع ملف ثم حفظه
  ========================== */
  const handleSubmit = async (e) => {
    ...
    if (isVip) {
      ...
      try {
        ...
        // reset البسيط
        // (ممكن متتمسحش عشان إنشاء متكرر سريع)
        // setClientName(""); setClientNumber(""); setVipSlug(""); setVipImageUrl("");

      } catch (err) {
        setMsg(lang === "ar" ? "خطأ أثناء إنشاء الصفحة!" : "Error creating page!");
      } finally {
        setUploading(false);
      }
      return;
    ...
    try {
      ...
      const uploadRes  = await fetch("/api/upload-to-gcs", { method: "POST", body: formData });
      const uploadJson = await uploadRes.json();
      if (!uploadJson.url) throw new Error("Upload failed");

      // 2) حفظ بيانات الملف
      const docRef = await addDoc(collection(firestore, "archiveFiles"), {
        nameAr, nameEn, descAr, descEn, category,
        link: uploadJson.url, createdAt: Timestamp.now(),
      });
      ...
      // reset حقول الرفع
      setFile(null); setNameAr(""); setNameEn(""); setDescAr(""); setDescEn("");
    } catch (err) {
  ...
  const handleDownloadQR = (qrKey) => {
    ...
    a.href = url; a.download = `taheel-qr-${name}.png`; a.click();
  };
    ...
    <div className="max-w-4xl mx-auto p-4 font-cairo">
      ...
      <div className="bg-[#171f26] rounded-xl shadow-lg p-5 mb-8 border border-emerald-800">
        <h2 className="font-extrabold text-2xl mb-6 text-emerald-400 text-center drop-shadow">
          {isVip
            ? (lang === "ar" ? "إنشاء صفحة عميل VIP" : "Create VIP Client Page")
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4 items-end">
          ...
          >
            {uploading
              ? (lang === "ar" ? "جارٍ التنفيذ..." : "Working...")
              : isVip
                ? (lang === "ar" ? "إنشاء الصفحة" : "Create Page")
      ...
      <div className="bg-[#1a272f] rounded-xl shadow-lg p-5 border border-emerald-900">
        ...
        {/* مكان عرض أي QR مختار من الجدول */}
        {qrFor && (
          <div
            ref={qrRef}
