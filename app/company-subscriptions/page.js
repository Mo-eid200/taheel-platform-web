...
/** ✅ Animated rotating border wrapper (for selected cards) */
function AnimatedBorder({ active, radius = "rounded-3xl", children }) {
  ...
  return (
    <div className={cn("relative", radius)}>
      ...
      <div
        ...
        style={{
          background:
            "conic-gradient(from 180deg, rgba(16,185,129,0.0), rgba(16,185,129,0.95), rgba(56,189,248,0.95), rgba(168,85,247,0.95), rgba(16,185,129,0.0))",
          filter: "blur(0px)",
        }}
      />
      {/* inner cover */}
      <div className={cn("absolute inset-0 z-0", radius, "bg-[#0b131e]")} />
      ...
      <div className="sticky top-0 z-50 backdrop-blur-xl bg-[#08101a]/70 border-b border-white/10">
        <div className="max-w-6xl mx-auto px-3 sm:px-4 py-3">
          <div className={cn("flex items-center justify-between", isArabic && "flex-row-reverse")}>
            <Link
              href={`/?lang=${lang}`}
              className="inline-flex items-center gap-2 text-white/85 hover:text-white font-extrabold text-sm"
            >
            ...
            <div className={cn("flex items-center gap-2", isArabic && "flex-row-reverse")}>
              <button
                onClick={toggleLang}
                className="px-3 py-2 rounded-full bg-white/8 border border-white/12 text-white/85 font-extrabold text-xs hover:bg-white/12 transition"
              >
                {t.langBtn}
              </button>
          ...
          {PACKAGES.map((p, idx) => {
            ...
            return (
              <motion.div
                ...
                className="relative"
              >
                <AnimatedBorder active={isOpen} radius="rounded-3xl">
                  <div
                    className={cn(
                      "relative rounded-3xl border bg-white/5 backdrop-blur-xl overflow-hidden",
                      p.brand.ring,
                      isOpen ? "border-emerald-400/45" : ""
                    )}
                  >
                    {/* identity bar */}
                        ...
                        >
                          <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
                            ...
                            <div className="lg:col-span-7">
                              ...
                              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                {p.durations.map((d) => {
                                  ...
                                  return (
                                    <AnimatedBorder key={d.key} active={active} radius="rounded-2xl">
                                      <button
                                        onClick={() => setDurationFor(p.key, d.key)}
                                        className={cn(
                                          "relative w-full rounded-2xl border p-4 bg-white/6 backdrop-blur-xl transition text-left cursor-pointer",
                                          active
                            ...
                            <div className="lg:col-span-5">
                              <AnimatedBorder active={true} radius="rounded-3xl">
